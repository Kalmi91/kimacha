...
const DEFAULT_PAIR = 'default';

class SQLiteDB implements DB {
  private db: SQLite.SQLiteDatabase | null = null;
  // Cached active language pair. Cards + level progress are scoped to it so a
  // new language always starts the level over instead of inheriting progress.
  ...
  private async open() {
    ...
    await this.db.execAsync(`
      CREATE TABLE IF NOT EXISTS cards (
        ...
        lang_pair TEXT NOT NULL DEFAULT 'default',
        due TEXT NOT NULL,
  ...
  // Schema migrations for pre-existing installs.
  private async migrate(db: SQLite.SQLiteDatabase) {
    // 1. buried column (older installs lack it). Must run before the lang_pair
    //    rebuild so the rebuilt table carries it.
    ...
    // 2. lang_pair on cards. Old installs have UNIQUE(word_id, type) baked into
    //    the table, which blocks per-pair rows — so we rebuild the table.
    const cardPairCol = await db.getFirstAsync<any>("SELECT * FROM pragma_table_info('cards') WHERE name = 'lang_pair'");
    if (!cardPairCol) {
      const existingPair = await this.getPair();
      await db.execAsync(`
      ...
      `);
      await db.runAsync(
        `INSERT INTO cards_new (id, word_id, type, lang_pair, due, stability, difficulty, elapsed_days, scheduled_days, learning_steps, reps, lapses, state, last_review, buried)
         SELECT id, word_id, type, ?, due, stability, difficulty, elapsed_days, scheduled_days, learning_steps, reps, lapses, state, last_review, buried FROM cards`,
        [existingPair]
      );
      await db.execAsync('DROP TABLE cards; ALTER TABLE cards_new RENAME TO cards;');
    }

    // 3. user_level keyed by lang_pair (old schema was single row id=1).
    const levelPairCol = await db.getFirstAsync<any>("SELECT * FROM pragma_table_info('user_level') WHERE name = 'lang_pair'");
    if (!levelPairCol) {
      const existingPair = await this.getPair();
      await db.execAsync(`
      ...
      `);
      await db.runAsync(
        `INSERT INTO user_level_new (lang_pair, level, correct_streak, mistakes_in_window, fail_streak)
         SELECT ?, level, correct_streak, mistakes_in_window, fail_streak FROM user_level WHERE id = 1`,
        [existingPair]
      );
      await db.execAsync('DROP TABLE user_level; ALTER TABLE user_level_new RENAME TO user_level;');
    }
  }

  // Resolve the active language pair, e.g. "es>hu". Cached; invalidated by
  // setOnboarding. Falls back to a sentinel before onboarding completes.
  private async getPair(): Promise<string> {
    if (this.pairCache) return this.pairCache;
    const ob = await this.getOnboarding();
    this.pairCache = ob ? `${ob.source}>${ob.target}` : DEFAULT_PAIR;
  ...
  async ensureCard(wordId: number, type: string) {
    ...
    await db.runAsync(
      `INSERT OR IGNORE INTO cards (word_id, type, lang_pair, due, stability, difficulty, elapsed_days, scheduled_days, learning_steps, reps, lapses, state)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [wordId, type, pair, empty.due.toISOString(), empty.stability, empty.difficulty, empty.elapsed_days, empty.scheduled_days, empty.learning_steps, empty.reps, empty.lapses, empty.state]
    );
  }

  // Bulk create word + sentence cards for many words in a single transaction.
  // Replaces the old per-word await loop (2 queries × N words on every load).
      ...
      try {
        for (const id of wordIds) {
          for (const type of ['word', 'sentence']) {
            await stmt.executeAsync([id, type, pair, due, empty.stability, empty.difficulty, empty.elapsed_days, empty.scheduled_days, empty.learning_steps, empty.reps, empty.lapses, empty.state]);
          }
        }
      } finally {
  ...
  async updateCard(wordId: number, type: string, card: Card) {
    ...
    await db.runAsync(
      `UPDATE cards SET due = ?, stability = ?, difficulty = ?, elapsed_days = ?, scheduled_days = ?,
       learning_steps = ?, reps = ?, lapses = ?, state = ?, last_review = ? WHERE word_id = ? AND type = ? AND lang_pair = ?`,
      [card.due.toISOString(), card.stability, card.difficulty, card.elapsed_days, card.scheduled_days,
       card.learning_steps, card.reps, card.lapses, card.state, card.last_review ? card.last_review.toISOString() : null, wordId, type, pair]
    );
  }
  ...
  async getDueCards(limit: number) {
    const db = await this.open();
    const pair = await this.getPair();
    return await db.getAllAsync('SELECT * FROM cards WHERE due <= ? AND buried = 0 AND lang_pair = ? ORDER BY due ASC LIMIT ?', [new Date().toISOString(), pair, limit]);
  }
  ...
  async setOnboarding(source: string, target: string) {
    ...
    this.pairCache = `${source}>${target}`;
  }

  async getLevel() {
    const db = await this.open();
    const pair = await this.getPair();
    let row = await db.getFirstAsync<any>('SELECT * FROM user_level WHERE lang_pair = ?', [pair]);
    if (!row) {
      // First time on this language pair → start at A0.
      await db.runAsync('INSERT OR IGNORE INTO user_level (lang_pair, level, correct_streak, mistakes_in_window, fail_streak) VALUES (?, ?, 0, 0, 0)', [pair, 'A0']);
      row = await db.getFirstAsync<any>('SELECT * FROM user_level WHERE lang_pair = ?', [pair]);
    }
    return row;
  }
  ...
  async updateLevel(level: string, correctStreak: number, mistakesInWindow: number, failStreak: number) {
    const db = await this.open();
    const pair = await this.getPair();
    await db.runAsync('INSERT OR REPLACE INTO user_level (lang_pair, level, correct_streak, mistakes_in_window, fail_streak) VALUES (?, ?, ?, ?, ?)',
      [pair, level, correctStreak, mistakesInWindow, failStreak]);
  }
  ...
  async getDueCardsForWordIds(wordIds: number[], limit: number) {
    ...
    const newCards = await db.getAllAsync(
      `SELECT * FROM cards WHERE word_id IN (${placeholders}) AND type = 'word' AND reps = 0 AND buried = 0 AND lang_pair = ? AND due <= ? ORDER BY due ASC LIMIT ?`,
      [...wordIds, pair, now, newLimit]
    );

    const reviewWords = await db.getAllAsync(
      `SELECT * FROM cards WHERE word_id IN (${placeholders}) AND type = 'word' AND reps > 0 AND buried = 0 AND lang_pair = ? AND due <= ? ORDER BY due ASC LIMIT ?`,
      [...wordIds, pair, lookahead, reviewLimit]
    );

    const reviewedWordIds = await db.getAllAsync<any>(
      `SELECT DISTINCT word_id FROM cards WHERE word_id IN (${placeholders}) AND type = 'word' AND lang_pair = ? AND (reps >= 2 OR buried = 1)`,
      [...wordIds, pair]
    );
    ...
    if (reviewedSet.size > 0) {
      ...
      sentenceCards = await db.getAllAsync(
        `SELECT * FROM cards WHERE word_id IN (${sentencePlaceholders}) AND type = 'sentence' AND buried = 0 AND lang_pair = ? AND due <= ? ORDER BY due ASC LIMIT ?`,
        [...reviewedIds, pair, lookahead, sentenceSlots]
      );
    }
  ...
  async getWordReps(wordIds: number[]): Promise<Map<number, number>> {
    ...
    const rows = await db.getAllAsync<any>(
      `SELECT word_id, reps FROM cards WHERE word_id IN (${placeholders}) AND type = 'word' AND lang_pair = ?`,
      [...wordIds, pair]
    );
  ...
  async getMasteredCount() {
    ...
    const row = await db.getFirstAsync<any>(
      "SELECT COUNT(*) as cnt FROM cards WHERE state >= 2 AND stability > 10 AND lang_pair = ?", [pair]
    );
    return row?.cnt ?? 0;
  }
  ...
  async getReviewedWordCount(level: string) {
    ...
    const placeholders = wordIds.map(() => '?').join(',');
    const row = await db.getFirstAsync<any>(
      `SELECT COUNT(*) as cnt FROM cards WHERE word_id IN (${placeholders}) AND type = 'word' AND reps > 0 AND lang_pair = ?`,
      [...wordIds, pair]
    );
    return row?.cnt ?? 0;
  }
  ...
  async buryCard(wordId: number, type: string) {
    const db = await this.open();
    const pair = await this.getPair();
    await db.runAsync('UPDATE cards SET buried = 1 WHERE word_id = ? AND type = ? AND lang_pair = ?', [wordId, type, pair]);
  }

  // Per-level mastered word count for the active pair: FSRS state >= Review (2)
  // and stability > 10 days. This is the exam gate (≠ getMasteredCount which is
  // a global analytics figure).
  async getMasteredWordCount(level: string) {
    ...
    const placeholders = wordIds.map(() => '?').join(',');
    const row = await db.getFirstAsync<any>(
      `SELECT COUNT(*) as cnt FROM cards WHERE word_id IN (${placeholders}) AND type = 'word' AND lang_pair = ? AND state >= 2 AND stability > 10`,
      [...wordIds, pair]
    );
    return row?.cnt ?? 0;
  }

  async recordExamAttempt(questionId: number, level: string, correct: boolean, responseTimeMs: number) {
    const db = await this.open();
    const pair = await this.getPair();
    await db.runAsync(
      'INSERT INTO exam_attempts (lang_pair, question_id, level, correct, response_time_ms, timestamp) VALUES (?, ?, ?, ?, ?, ?)',
      [pair, questionId, level, correct ? 1 : 0, responseTimeMs, new Date().toISOString()]
    );
  }

  // question_id → most recent result (true=correct). Absent = never attempted.
  async getExamLastResults(level: string) {
    const db = await this.open();
    const pair = await this.getPair();
    const rows = await db.getAllAsync<any>(
      'SELECT question_id, correct FROM exam_attempts WHERE level = ? AND lang_pair = ? ORDER BY timestamp DESC, id DESC',
      [level, pair]
    );
    const map = new Map<number, boolean>();
    for (const r of rows) {
      if (!map.has(r.question_id)) map.set(r.question_id, r.correct === 1);
    }
    return map;
  }
  ...
  async recordExamSession(level: string, total: number, correct: number, passed: boolean) {
    ...
    const pair = await this.getPair();
    await db.runAsync(
      'INSERT INTO exam_sessions (lang_pair, level, total, correct, passed, timestamp) VALUES (?, ?, ?, ?, ?, ?)',
      [pair, level, total, correct, passed ? 1 : 0, new Date().toISOString()]
    );
  }
  ...
  async getExamSummary() {
    ...
    const pair = await this.getPair();
    const rows = await db.getAllAsync<any>(
      'SELECT level, passed FROM exam_sessions WHERE lang_pair = ? ORDER BY timestamp DESC, id DESC',
      [pair]
    );
    return {
      examsTaken: rows.length,
      examsPassed: rows.filter((r: any) => r.passed === 1).length,
      lastExamLevel: rows[0]?.level ?? null,
    };
  }
}
