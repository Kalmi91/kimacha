...
export default function ExamWordCard({ item, onResult }: Props) {
  ...
  const handleCheck = () => {
    if (!text.trim()) return;
    const typed = text.trim().toLowerCase().replace(/[¡¿]/g, '').replace(/[.]+$/, '').trim();
    const expected = item.answer.toLowerCase().replace(/[¡¿]/g, '').replace(/[.]+$/, '').trim();
