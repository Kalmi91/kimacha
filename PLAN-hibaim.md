...
- [x] 6. Skill a mai doksin → `hibaim-2026-09-23-claude.json` a Drive-on → kész, ha: a validátor elfogadja (2026-09-23 20:10 CST: `OK: 54 sentences, 28 words, 36 drills, 14 wrongWords`, 3 doubtful; Drive `kimacha/hibaim` 1xy_zR9WanCDjvvpl3zWUhnkXCM8QGFJD, feltöltő `~/code/hibaim_upload.py`)
- [!] 7. Web-export képernyőképek (import, riport, kártya) → kész, ha: Kálmán megnézte (2026-09-23 21:xx CST: a sandboxban az agent-browser lapja kattintásra about:blank-re esik (renderer, nem app-hiba: sandboxon kívül az onboarding végigment); a sandboxon kívüli futtatást az auto-mód elutasította, Kálmán döntése kell: engedély vagy egyből APK; Kálmán kivette az agent-browsert a sandboxból, a `--disable-dev-shm-usage` az onboarding-összeomlást megoldotta, de a Settings fül így is about:blank, ok ismeretlen → telefonos próba)
